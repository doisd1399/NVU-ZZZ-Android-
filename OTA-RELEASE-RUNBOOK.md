...
O Web e o APK devem ser gerados a partir do mesmo commit. O build Web de produção usa a versão `2.3.141`, o runtime `R3.34-PC-WEB-AUTH-ARCHITECTURE-MODULAR` e o canal nativo `production-359`. Nunca publique um `dist` gerado de outra cópia do projeto.
...
O manifesto precisa conter `artifactType: zip`, `bundleId: production-359-<webVersion>`, `webVersion`, `runtimeRevision`, `nativeChannel: production-359`, `nativeVersionCode` igual ao `versionCode` do APK, `downloadUrl` HTTPS na mesma origem do manifesto, checksum SHA-256 do ZIP e `signature` RSA válida.
