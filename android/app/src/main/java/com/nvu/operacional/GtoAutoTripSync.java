...
final class GtoAutoTripSync {
    ...
    static boolean restoreLockedFreightToPrefs(Context context, SharedPreferences prefs, String sessionId) {
        ...
        try {
            ...
            for (String field : new String[] {
                "cargo", "companyRoute", "originCompany", "destinationCompany",
    ...
    static boolean lockSelectedFreight(Context context, SharedPreferences prefs) {
        ...
        try {
            ...
            // The GTO card's source company is the requested business-semantic Origem.
            // Force it before hashing so display, durable snapshot and backend payload
            // share exactly the same immutable value.
            candidate.put("origin", clean(candidate.optString("originCompany", "")));
    ...
    private static JSONObject buildPayload(Context context, SharedPreferences prefs, String sessionId) {
        ...
        try {
            ...
            // Reassert the invariant at transmission time as defense in depth against
            // stale legacy snapshots created before HF2.
            payload.put("origin", clean(payload.optString("originCompany", "")));
            ...
            // Compatibility aliases are additive only. HF2 intentionally defines the
            // detected source company as the canonical Origem for automatic GTO trips.
    ...
    private static String validateFreight(JSONObject payload, String contractMode) {
        ...
        if (origin.length() < 2 || origin.length() > 220 || !origin.equals(originCompany)) {
            return "Dado de frete inválido: origin deve corresponder à empresa de origem.";
        }
