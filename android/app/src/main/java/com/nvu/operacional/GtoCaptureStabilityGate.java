 ...
 *
 * The gate deliberately requires both fresh GTO foreground evidence and a short
 * settling interval. Three frames alone can arrive in less than one display refresh
 * burst and would otherwise allow the portrait NVU surface to become "ready" just
 * before the game switches to landscape.
 */
final class GtoCaptureStabilityGate {
    ...
    synchronized Snapshot observeFrame(
        ...
        int height,
        long now,
        boolean hasFreshGtoForegroundEvidence
    ) {
        boolean wasReady = ready;

        if (!hasFreshGtoForegroundEvidence) {
            ready = false;
            phase = CAPTURE_WAITING_GTO_FOREGROUND;
            stableWidth = 0;
            stableHeight = 0;
            stableFrames = 0;
            return snapshot(false, wasReady);
