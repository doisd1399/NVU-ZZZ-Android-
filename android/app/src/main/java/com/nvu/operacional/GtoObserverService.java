...
public class GtoObserverService extends Service {
    ...
    public void onCreate() {
        ...
        if (recoverCompletedTrip) {
            ...
            prefs.edit()
                ...
                .putString("lastEvent", "Entrega certificada recuperada · registro automático será retomado no GTO")
                .apply();
    ...
    private void resolveCertifiedResultExitFromFrames(long generation) {
        ...
        if (screenAnalysisPausedOutsideGto || !gtoForeground) {
            prefs.edit().putString("resultTerminalResolverStatus", "WAITING_GTO_FOREGROUND").apply();
            mainHandler.postDelayed(() -> resolveCertifiedResultExitFromFrames(generation), 350L);
            return;
        }
        ...
        if (!eligible) {
            ...
            prefs.edit().putString("resultTerminalResolverStatus", "WAITING_SAFE_NO_AD_WINDOW").apply();
            mainHandler.postDelayed(() -> resolveCertifiedResultExitFromFrames(generation), Math.min(remaining, 1200L));
            return;
        }

        prefs.edit()
            .putString("resultTerminalResolverStatus", "RESOLVING_NORMAL")
    ...
    private void latchCertifiedResultExitAndSend(long now, String source) {
        ...
        if (!prefs.getBoolean("resultCertifiedLatched", false)
            || prefs.getBoolean("resultWatchedAdEvidence", false)) return;
    ...
    private void armCompletionQueueSealRecovery(String sessionId, long now) {
        ...
        prefs.edit()
            ...
            .putLong("gtoCompletionQueueSealRetryAt", now + delay)
    ...
    private void handleOcrResult(Text text, float analysisScale, int analysisOffsetX, int analysisOffsetY, Bitmap fullFrame) {
        ...
        if (resultScreen != null) {
            ...
            // Every certified result requires an ADS guard. Geometry has an OCR-free
            // fallback, so missing the ADS label can never reopen the post-result race.
            boolean adsGuardRequired = true;
    ...
    private void confirmNormalResultAutomatically() {
        ...
        if ((!receiveLatch && !automaticResultLatch) || !certifiedProof || watchedAdEvidence) {
            prefs.edit()
