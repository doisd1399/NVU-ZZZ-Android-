...
check("field review preserves confirmed selection", service.includes('putString("selectionIdentityStatus", "CONFIRMED")') && service.includes('putString("selectionConfirmationStatus", "REVIEW_REQUIRED")') && service.includes("enterFreightReview"));
...
check("manual destination is not silently passed through trusted city resolver", !service.includes("finalDestination = resolveTrustedDestination(canonical.destination)") && service.includes("it never autocorrects or silently rewrites names"));
