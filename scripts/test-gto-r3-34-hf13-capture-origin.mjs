...
check("white health dot is driven by real pipeline health",
  service.includes("captureHealthDotView") && service.includes("healthy ? Color.WHITE") && service.includes('"HEALTHY_REAL_FRAMES"'));
...
check("informational list detection does not relax active-trip replacement policy",
  service.includes("informationalListProbe") && flow.includes('if ("TRIP_IN_PROGRESS".equals(state)) return explicitReplacementArmed;'));
