...
check("route OCR cadence remains real-time semantic authority while visual exit is post-result only",
  resultFallbackMs >= 180 && resultFallbackMs <= 500
    && service.includes("tripCandidateOcrDue = false")
    && service.includes("if (resultTrackingState && resultScreenLastSeenAt > 0L)")
