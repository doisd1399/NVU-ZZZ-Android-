...
ck("HF59 Android identity", versionCode === 111 && versionName === "1.0.111");
ck("HF59 workflow identity", workflow.includes('EXPECTED_VERSION_CODE: "111"') && workflow.includes('EXPECTED_VERSION_NAME: "1.0.111"'));
ck("HF59 release artifact identity", workflow.includes("NVU-R3.34-PC-HF59-release.apk"));
...
ck("missing native auth pauses locally without callable storm", sync.includes('pauseRetryForReason(retry, queuedSessionId, "NO_NATIVE_AUTH")') && sync.includes("FirebaseAuth.getInstance().getCurrentUser()"));
ck("repeated no-auth polls are locally idempotent", sync.includes("boolean anyPauseChanged = false") && sync.includes("if (listener != null && pauseChanged)") && sync.includes("blockReason.equals(existingReason) && existingRetryAt == Long.MAX_VALUE"));
...
ck("watchdog retries durable enqueue and finalizes proof only after success", service.includes("boolean queued = GtoAutoTripSync.enqueueConfirmedTrip(this, prefs, automaticTripSyncListener());") && service.includes("if (queued) {\n            finalizeResultProofAfterQueueSeal(sessionId);"));
