...
check("selected-row OCR never silently rewrites destination spelling", !service.includes("finalDestination = resolveTrustedDestination(canonical.destination)") && service.includes("it never autocorrects or silently rewrites names"));
check("unresolved destination conflict becomes field review without reselection", service.includes("hasUnresolvedDestinationOneEditConflict") && service.includes("A cidade de destino teve duas leituras diferentes. A NVU não vai adivinhar o nome") && service.includes("GtoFreightReviewPolicy.DESTINATION"));
