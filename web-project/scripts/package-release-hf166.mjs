...
const artifacts = path.join(root, "release-artifacts-hf166");
const sourceZip = path.join(artifacts, "NVU-Operacional-1.0.243-HF166-Dev-SAFE.zip");
const distZip = path.join(artifacts, "NVU-Operacional-1.0.243-HF166-dist-Netlify.zip");
const apk = path.join(artifacts, "NVU-R3.34-PC-HF166-release.apk");
const report = path.join(artifacts, "NVU-R3.34-PC-HF166-release-report.md");
...
if (manifest.runtimeRevision !== "R3.34-PC-HF166" || manifest.capacitorRuntime !== "local") {
  throw new Error("Manifesto não corresponde ao runtime local HF166.");
}
...
if (!fs.existsSync(previousSignedApk)) {
  throw new Error("APK-base HF164 ausente; a origem do APK HF166 deve ser o Release recém-assinado.");
}
const generatedApk = "/tmp/nvu-hf166-release/NVU-R3.34-PC-HF166-release.apk";
...
const reportText = `# Relatório técnico — NVU R3.34-PC-HF166 / Android 1.0.243
...
| Versão | \`1.0.243\` / versionCode \`243\` |
...
| Gate HF166 | \`15\` verificações estruturais de autenticação/autorização aprovadas |
...
No login Android, o plugin 8.4.0 usa Credential Manager por padrão. A versão HF166 mantém esse caminho como preferencial e, para falhas de provider/Play Services/compatibilidade que não sejam cancelamento, usa o fallback público legado GoogleSignInClient do próprio plugin. O idToken nativo é trocado por credencial Firebase JS e confirmado antes de liberar a sessão. Android nunca usa popup, redirect ou Chrome.

No RH, a causa comprovada era a divergência entre a UI, que podia reconhecer admin pelo vínculo companyMembers, e Rules/escritas client-side, que exigiam users.role/roles. As quatro ações críticas agora são callables server-authoritative, autorizadas no servidor por owner/admin membership/claim sênior e aplicadas em transações idempotentes, incluindo usuário, vínculo, simulator_members, solicitação e notificação.

No Painel Sênior, o claim é renovado no limite da entrada, a empresa é carregada/primada, o escopo é persistido e switchRole é aguardado antes da navegação. Isso elimina a corrida entre activeCompanyId, listeners e rota, preservando a precedência de escopo explícito.

As correções GTO HF159–HF165 permanecem cobertas pelos gates históricos; Carga/Origem/Destino, Lista/Pause, snapshot, envio automático e card não foram reescritos.
...
[4]: scripts/test-gto-r3-34-hf166-authz.mjs "Gate HF166 de autenticação e autorização"
`;
...
console.log(`Pacote HF166 criado: ${path.basename(sourceZip)}, ${path.basename(distZip)}, ${path.basename(report)} e ${path.basename(checksums)}.`);
