...
const artifacts = path.join(root, "release-artifacts-hf167");
const sourceZip = path.join(artifacts, "NVU-Operacional-1.0.244-HF167-Dev-SAFE.zip");
const distZip = path.join(artifacts, "NVU-Operacional-1.0.244-HF167-dist-Netlify.zip");
const apk = path.join(artifacts, "NVU-R3.34-PC-HF167-release.apk");
const report = path.join(artifacts, "NVU-R3.34-PC-HF167-release-report.md");
...
if (manifest.runtimeRevision !== "R3.34-PC-HF167" || manifest.capacitorRuntime !== "local") {
  throw new Error("Manifesto não corresponde ao runtime local HF167.");
}
...
if (!fs.existsSync(previousSignedApk)) {
  throw new Error("APK-base HF166 ausente; a origem do APK HF167 deve ser o Release recém-assinado.");
}
const generatedApk = "/tmp/nvu-hf167-release/NVU-R3.34-PC-HF167-release.apk";
...
for (const required of [
  ...
  "scripts/test-gto-r3-34-hf167-freight-pause.mjs",
...
const reportText = `# Relatório técnico — NVU R3.34-PC-HF167 / Android 1.0.244
...
| Versão | \`1.0.244\` / versionCode \`244\` |
...
| Gate HF167 | \`9\` verificações estruturais de Lista/Pause aprovadas |
...
No Pause, a causa comprovada era contratual: 'pauseLocationTextField' chamava 'extractAfterLastSeparator', retornando somente o sufixo compartilhado 'Área Rural'. Origem e Destino podiam então ficar iguais, e 'firstRequiredField' rejeitava a viagem como rota colapsada. O novo 'extractOperationalLocation' usa o prefixo antes do último separador para identidade operacional; o parser de sufixo continua disponível somente para diagnóstico.
...
[4]: scripts/test-gto-r3-34-hf167-freight-pause.mjs "Gate HF167 de Lista e Pause"
`;
...
console.log(`Pacote HF167 criado: ${path.basename(sourceZip)}, ${path.basename(distZip)}, ${path.basename(report)} e ${path.basename(checksums)}.`);
